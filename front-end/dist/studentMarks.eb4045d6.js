...
})({"iYlYu":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "5497fae7eb4045d6";
"use strict";
...
},{}],"6H3Sr":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
var _jquery = require("jquery");
var _jqueryDefault = parcelHelpers.interopDefault(_jquery);
var _mainJs = require("./main.js");
const yearSelection = (0, _jqueryDefault.default)("#yearSelect");
const semesterSelection = (0, _jqueryDefault.default)("#semesterSelect");
const gradeSelection = (0, _jqueryDefault.default)("#gradeSelect");
const classSelection = (0, _jqueryDefault.default)("#classSelect");
const subjectSelection = (0, _jqueryDefault.default)("#subjectSelect");
const indexNoInputElm = (0, _jqueryDefault.default)("#indexNoInput");
const marksInputElm = (0, _jqueryDefault.default)("#marksInput");
const btnSave = (0, _jqueryDefault.default)("#save");
const btnClear = (0, _jqueryDefault.default)("#clear");
...
(0, _jqueryDefault.default)("tbody").on("click", ".delete", (eventData)=>{
    const id = +(0, _jqueryDefault.default)(eventData.target).parents("tr").children("td:first-child").text();
    const xhr = new XMLHttpRequest();
    xhr.onload = function() {
        if (xhr.status === 204) {
            (0, _mainJs.showToast)("success", "Deleted", "studentMark has been deleted successfully");
            (0, _jqueryDefault.default)(eventData.target).parents("tr").remove();
            getStudentMarks();
        } else (0, _mainJs.showToast)("error", "Failed", "Failed to delete the studentMark, try again!");
    };
    xhr.open("DELETE", `http://localhost:8080/app/studentmark/${id}`, true);
    xhr.send();
});
(0, _jqueryDefault.default)("tbody").on("click", ".edit", (eventData)=>{
    btnSave.text("Update");
    const tdList = (0, _jqueryDefault.default)(eventData.target).parents("tr").children("td");
    const abc = [];
    for (const tdListElement of tdList)abc.push((0, _jqueryDefault.default)(tdListElement).text());
...
btnSave.on("click", ()=>{
    ...
    if (/^.+$/.test(studentIndexNo)) {
        if (/^\d{1,3}$/.test(mark)) {
                ...
                xhr.addEventListener("readystatechange", function() {
                    if (xhr.readyState === 4) {
                        if (xhr.status === 201) {
                            ...
                            yearSelection.focus();
                            location.reload();
                            (0, _mainJs.showToast)("success", "Saved", "student mark saved successfully");
                        } else {
                            const errorObj = JSON.parse(xhr.responseText);
                            (0, _mainJs.showToast)("error", "Failed to save", errorObj.message);
                        }
                    }
                });
            ...
            mark.select();
            (0, _mainJs.showToast)("warning", "please enter correct mark", undefined);
        }
    } else {
        studentIndexNo.select();
        (0, _mainJs.showToast)("warning", "please enter correct student index no", undefined);
    }
});
function getStudentMarks() {
    ...
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                (0, _jqueryDefault.default)("tbody").empty();
                (0, _jqueryDefault.default)("tfoot").hide();
                const studentMarkList = JSON.parse(xhr.responseText);
                studentMarkList.forEach((studentMark)=>{
                    (0, _jqueryDefault.default)("table>tbody").append(` <tr>
    ...
    const searchText = (0, _jqueryDefault.default)("#searchInput").val().trim();
    const query = searchText ? `?q=${searchText}` : "";
    xhr.open("GET", `http://localhost:8080/app/studentmark${query}`, true);
    xhr.addEventListener("loadstart", ()=>(0, _jqueryDefault.default)("tfoot").text("Please wait!"));
    xhr.addEventListener("loadend", ()=>(0, _jqueryDefault.default)("tfoot").text("No studentMarks  are found!"));
    xhr.send();
}
getStudentMarks();
(0, _jqueryDefault.default)("#searchInput").on("input", (event)=>{
    getStudentMarks();
});
function getYears() {
    const xhr = new XMLHttpRequest();
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (!yearList.length) yearSelection.append(`<option>Please enter years to show</option>`);
            } else {
                yearSelection.empty();
                yearSelection.append(`<option>Please enter years to show</option>`);
                (0, _mainJs.showToast)("error", "Failed", "Failed to load years");
                console.log(JSON.parse(xhr.responseText));
            }
        }
    });
    ...
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (!semesterList.length) semesterSelection.append(`<option>Please enter semesters to show</option>`);
            } else {
                semesterSelection.empty();
                semesterSelection.append(`<option>Please enter semesters to show</option>`);
                (0, _mainJs.showToast)("error", "Failed", "Failed to load semesters");
                console.log(JSON.parse(xhr.responseText));
            }
        }
    });
    ...
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (!gradeList.length) gradeSelection.append(`<option>Please enter grades to show</option>`);
            } else {
                gradeSelection.empty();
                gradeSelection.append(`<option>Please enter grades to show</option>`);
                (0, _mainJs.showToast)("error", "Failed", "Failed to load grades");
                console.log(JSON.parse(xhr.responseText));
            }
        }
    });
    ...
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (!classList.length) classSelection.append(`<option>Please enter classes to show</option>`);
            } else {
                classSelection.empty();
                classSelection.append(`<option>Please enter classes to show</option>`);
                (0, _mainJs.showToast)("error", "Failed", "Failed to load classes");
                console.log(JSON.parse(xhr.responseText));
            }
        }
    });
    ...
    xhr.addEventListener("readystatechange", ()=>{
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
                ...
                if (!subjectList.length) subjectSelection.append(`<option>Please enter subjects to show</option>`);
            } else {
                subjectSelection.empty();
                subjectSelection.append(`<option>Please enter subjects to show</option>`);
                (0, _mainJs.showToast)("error", "Failed", "Failed to load subjects");
                console.log(JSON.parse(xhr.responseText));
            }
        }
    });
...
},{"jquery":"hgMhh","./main.js":"1SICI","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"gkKU3":[function(require,module,exports) {
...
},{}]},["iYlYu","6H3Sr"], "6H3Sr", "parcelRequire8d29")

//# sourceMappingURL=studentMarks.eb4045d6.js.map
